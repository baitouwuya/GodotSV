...
func run_test_1_read_csv() -> void:
	...
	test_count += 1
	var test_name := "读取 CSV 文件 (tests/compatibility.csv)"
	var test_path := "res://tests/compatibility.csv"
...
func run_test_2_read_tsv() -> void:
	...
	test_count += 1
	var test_name := "读取 TSV 文件 (tests/compatibility.tsv)"
	var test_path := "res://tests/compatibility.tsv"
...
func run_test_3_read_gdsv() -> void:
	...
	test_count += 1
	var test_name := "读取 GDSV 文件 (tests/basic.gdsv)"
	var test_path := "res://tests/basic.gdsv"
...
func run_test_4_csv_to_gdsv() -> void:
	...
	var csv_path := "res://tests/compatibility.csv"
...
func run_test_5_tsv_to_gdsv() -> void:
	...
	var tsv_path := "res://tests/compatibility.tsv"
...
func run_test_6_data_consistency() -> void:
	...
	TestOutputLogger.log(" CSV 文件: res://tests/compatibility.csv")
	TestOutputLogger.log(" TSV 文件: res://tests/compatibility.tsv\n")

	# 加载 CSV
	processor.reset()
	var csv_success := processor.load_gdsv_file("res://tests/compatibility.csv")
	if not csv_success:
	...
	# 加载 TSV
	processor.reset()
	var tsv_success := processor.load_gdsv_file("res://tests/compatibility.tsv")
	if not tsv_success:
