...
func test4_multi_column_search() -> void:
	...
	TestOutputLogger.log("  性能差异: %.2fx" % (multi_time_ms / single_time_ms if single_time_ms > 0 else 0.0))
...
func test5_case_sensitivity_comparison() -> void:
	...
	TestOutputLogger.log("  性能差异: %.2fx" % (insensitive_time_ms / sensitive_time_ms if sensitive_time_ms > 0 else 0.0))
