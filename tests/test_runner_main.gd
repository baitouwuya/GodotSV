...
## 测试分类配置，每个分类可包含多个测试场景
var test_categories: Array[Dictionary] = [
	{
		"name": "I/O 功能测试",
		"scene_paths": [
			"res://tests/basic_io/basic_io_test.tscn",
			"res://tests/multi_format/multi_format_test.tscn",
		],
		"description": "基础读写、类型转换、多格式兼容性 (CSV/TSV/GDSV)"
	},
	{
		"name": "类型系统测试",
		"scene_paths": [
			"res://tests/type_optimization/type_optimization_test.tscn",
			"res://tests/type_annotation/type_annotation_test.tscn",
			"res://tests/test_type_handler_system.tscn",
			"res://tests/test_custom_type_handler.tscn",
		],
		"description": "类型转换正确性、类型注解解析、类型处理器注册、自定义类型"
	},
	{
		"name": "搜索功能测试",
		"scene_paths": [
			"res://tests/search/simple_search_test.tscn",
		],
		"description": "搜索功能验证"
	},
	{
		"name": "性能基准测试",
		"scene_paths": [
			"res://tests/profiling/profiling_test.tscn",
			"res://tests/large_file/large_file_test.tscn",
			"res://tests/search_performance/search_performance_test.tscn",
		],
		"description": "解析性能、大文件读写、搜索性能基准"
	},
]
#endregion
...
func _on_test_category_clicked(category: Dictionary) -> void:
	...
	var scene_paths: Array = category.get("scene_paths", [])
	if scene_paths.is_empty():
		_append_output(COLOR_FAIL, "[Error] No test scenes configured for: %s" % category.name)
		return

	# 验证所有场景文件存在
	for scene_path: String in scene_paths:
		if not FileAccess.file_exists(scene_path):
			_append_output(COLOR_FAIL, "[Error] Test scene not found: %s" % scene_path)
			return
...
func _run_test(scene_path: String, reset_stats: bool = false) -> void:
	if reset_stats:
		_reset_test_state()
	is_running = true
	if start_time == 0.0:
	...
	# Wait for test scene to execute (most tests complete within a few frames)
	for frame_i in 5:
		await get_tree().process_frame

	# Signal completion without calling _mark_test_completed()
	# The caller is responsible for calling _mark_test_completed() once
	is_running = false


func _wait_for_test_completion() -> void:
	## 等待当前测试场景完成（is_running 被 _run_test 设为 false），不主动调用 _mark_test_completed
