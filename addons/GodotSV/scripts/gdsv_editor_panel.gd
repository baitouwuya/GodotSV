...
var _cell_input_custom: Control # 自定义编辑器（来自 GDSVEditorRegistry）
...
func _refresh_cell_input_from_model() -> void:
	...
	_cell_input_resource_btn.visible = false
	# 清理上一次的自定义编辑器
	if _cell_input_custom and is_instance_valid(_cell_input_custom):
		_cell_input_custom.queue_free()
		_cell_input_custom = null

	var t_lower := t.to_lower()

	# 优先检查 GDSVEditorRegistry 自定义编辑器
	# 先确保 GDScript 类型处理器已扫描（其 _init 会注册自定义编辑器）
	var registry := GDSVTypeHandlerRegistry.get_singleton()
	if registry:
		registry.scan_script_types()
	if GDSVEditorRegistry.has_editor(t_lower):
		var config := type_def.duplicate()
		config["initial_value"] = str(current)
		config["row"] = _active_cell.x
		config["column"] = _active_cell.y
		var editor: Control = GDSVEditorRegistry.create_editor(t_lower, _active_cell.x, _active_cell.y, config)
		if editor:
			_cell_input_line.visible = false
			editor.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			_cell_input_line.get_parent().add_child(editor)
			_cell_input_custom = editor
			# 连接自定义编辑器的 value_changed 信号以自动应用值
			if editor.has_signal("value_changed"):
				editor.value_changed.connect(_apply_cell_input_value)
...
func _get_input_raw_text() -> String:
	# 自定义编辑器优先
	if _cell_input_custom and is_instance_valid(_cell_input_custom) and _cell_input_custom.visible:
		if _cell_input_custom.has_method("get_value"):
			return str(_cell_input_custom.get_value())
