...
## 缓存的正则表达式
var _regex_number_list: RegEx = RegEx.create_from_string("[-+]?\\d*\\.?\\d+(?:[eE][-+]?\\d+)?")
var _regex_int_list: RegEx = RegEx.create_from_string("[-+]?\\d+")
var _regex_pattern_cache: Dictionary = {}
...
func validate_cell(row: int, column: int, value: Variant) -> Dictionary:
	...
	if not pattern.is_empty():
		if not _regex_pattern_cache.has(pattern):
			var regex: RegEx = RegEx.new()
			regex.compile(pattern)
			_regex_pattern_cache[pattern] = regex
		var regex: RegEx = _regex_pattern_cache[pattern]
...
func _parse_number_list(text: String) -> Array[float]:
	...
	if cleaned.is_empty():
		return []
	var out: Array[float] = []
	for m in _regex_number_list.search_all(cleaned):
		var s := m.get_string()
		if s.is_valid_float():
...
func _parse_int_list(text: String) -> Array[int]:
	...
	if cleaned.is_empty():
		return []
	var out: Array[int] = []
	for m in _regex_int_list.search_all(cleaned):
