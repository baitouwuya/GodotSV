...
func _draw_header() -> void:
	...
	var font := get_theme_font("font", "Label")
	var font_size := get_theme_font_size("font_size", "Label")
	var font_height: float = _get_font_height(font, font_size)
	var ascent: float = _get_font_ascent(font, font_size)
	var text_baseline_y: float = (float(header_height) - font_height) / 2.0 + ascent
	...
	for i in range(header_count):
		...
		var type_def: Dictionary = _get_type_definition_for_column(i)
		var type_name: String = str(type_def.get("type", ""))
		if not type_name.is_empty():
			var icon: Texture2D = _get_cached_type_icon(type_name)
			if icon:
				var icon_size: float = minf(16.0, float(header_height) - TEXT_PADDING_Y * 2.0)
				var icon_x: float = x_offset + TEXT_PADDING_X
				var icon_y: float = (float(header_height) - icon_size) / 2.0
				draw_texture_rect(icon, Rect2(icon_x, icon_y, icon_size, icon_size), false)
				icon_width = icon_size + 3.0

		# 绘制列标题文本（垂直居中）
		var text := header[i] if i < header.size() else ""
		var text_x: float = x_offset + icon_width + TEXT_PADDING_X
		var max_text_width: float = maxf(0.0, float(width) - icon_width - TEXT_PADDING_X * 2.0)
		var fitted_text: String = _fit_text_ellipsis(font, font_size, text, max_text_width)
		draw_string(font, Vector2(text_x, text_baseline_y), fitted_text, HORIZONTAL_ALIGNMENT_LEFT, max_text_width, font_size)
...
func set_data_model(model: GDSVDataModel) -> void:
	# 断开旧信号
	if data_model and data_model.data_changed.is_connected(_on_data_changed):
		data_model.data_changed.disconnect(_on_data_changed)
...
func set_schema_manager(manager: SchemaManager) -> void:
	# 断开旧信号
	if schema_manager and schema_manager.type_definitions_changed.is_connected(_on_type_definitions_changed):
		schema_manager.type_definitions_changed.disconnect(_on_type_definitions_changed)
...
func set_validation_manager(manager: ValidationManager) -> void:
	...
	if validation_manager:
		if validation_manager.error_added.is_connected(_on_error_added):
			validation_manager.error_added.disconnect(_on_error_added)
		if validation_manager.error_removed.is_connected(_on_error_removed):
			validation_manager.error_removed.disconnect(_on_error_removed)
		if validation_manager.errors_cleared.is_connected(_on_errors_cleared):
			validation_manager.errors_cleared.disconnect(_on_errors_cleared)
...
func _try_open_popup_editor(row: int, column: int) -> bool:
	...
	match t:
		...
		"vector3":
			...
			return true
		"vector4":
			_open_vector_dialog(row, column, 4)
			return true
		"rect2":
			_open_vector_dialog(row, column, 4)
			return true
...
func _open_vector_dialog(row: int, column: int, dimensions: int) -> void:
	...
	# Get editor config from C++ handler if available
	var registry := GDSVTypeHandlerRegistry.get_singleton()
	if registry and type_def:
		var data_type := str(type_def.get("type", "")).to_lower()
		if registry.has_type(data_type):
			var handler: GDSVTypeHandler = registry.get_type(data_type)
			if handler:
				editor_config = handler.GetEditorConfig()

	# Use custom labels if provided, otherwise default to X,Y,Z,W
	var default_labels := PackedStringArray(["X", "Y", "Z", "W"])
	var labels: Array = editor_config.get("labels", default_labels)
	var title := "编辑 %s" % str(type_def.get("type", "Vector" + str(dimensions)))
	...
	for i in range(dimensions):
		...
		var label := Label.new()
		label.text = str(labels[i]) if i < labels.size() else default_labels[i]
...
func _finish_editing() -> void:
	...
	match _current_editor_type:
		...
		"CustomEditor":
			# Custom editors can implement get_value() via:
			# 1. Method: editor.get_value()
			# 2. Metadata: editor.set_meta("get_value", callable)
			# 3. Fallback: LineEdit.text or TextEdit.text
			if _smart_editor:
				# Priority 1: Check for metadata callable
				if _smart_editor.has_meta("get_value"):
					var getter: Callable = _smart_editor.get_meta("get_value")
					if getter.is_valid():
						new_value = str(getter.call())
				# Priority 2: Check for method
				elif _smart_editor.has_method("get_value"):
					new_value = str(_smart_editor.get_value())
				# Priority 3: Fallback to standard controls
				elif _smart_editor is LineEdit:
					new_value = (_smart_editor as LineEdit).text
				elif _smart_editor is TextEdit:
					new_value = (_smart_editor as TextEdit).text
...
func _create_smart_editor(row: int, column: int, type_def: Dictionary) -> void:
	...
	# Priority 1: Custom registered GDScript editors
	if GDSVEditorRegistry.has_editor(data_type):
		# Build config with type_def + current value
		var config := type_def.duplicate()
		config["initial_value"] = data_model.get_cell_value(row, column) if data_model else ""
		config["row"] = row
		config["column"] = column

		var editor := GDSVEditorRegistry.create_editor(data_type, row, column, config)
		if editor:
			_smart_editor = editor
			_smart_editor.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
			...
			_current_editor_type = "CustomEditor"
			return

	# Priority 2: C++ handler-specified editors
	var registry := GDSVTypeHandlerRegistry.get_singleton()
	if registry and registry.has_type(data_type):
		var handler: GDSVTypeHandler = registry.get_type(data_type)
		if handler:
			var editor_type: String = handler.GetInlineEditorType()
			var editor_config: Dictionary = handler.GetEditorConfig()

			# Route to appropriate editor based on C++ handler recommendation
			match editor_type:
				"spinbox":
					_create_number_editor(row, column, editor_config.get("step", 1.0) == 1.0)
				"checkbox":
					_create_bool_editor(row, column, type_def)
				"lineedit":
					_create_line_editor(row, column)
				"vector_dialog":
					# Vector dialog is handled by double-click in _try_open_popup_editor
					_create_line_editor(row, column)
				"color_dialog":
					# Color dialog is handled by double-click in _try_open_popup_editor
					_create_line_editor(row, column)
				"resource_picker":
	...
	# Priority 3: Fallback to legacy hardcoded type matching for backward compatibility
	match data_type:
		"int":
...
## 获取缓存的类型图标（避免每帧查询 EditorInterface）
func _get_cached_type_icon(type_name: String) -> Texture2D:
	if _type_icon_cache.has(type_name):
		return _type_icon_cache[type_name]
	# 通过 Registry 获取规范类型名（如 "string" → "String"）
	var icon_name: String = type_name
	var registry := GDSVTypeHandlerRegistry.get_singleton()
	if registry:
		var handler: GDSVTypeHandler = registry.get_type(type_name)
		if handler:
			icon_name = str(handler.get_type_name())
	var icon: Texture2D = _get_editor_type_icon(icon_name)
	_type_icon_cache[type_name] = icon
	return icon
...
func _get_editor_type_icon(type_name: String) -> Texture2D:
	...
	if type_name == "enum" and theme.has_icon("Enum", "EditorIcons"):
		return theme.get_icon("Enum", "EditorIcons")
	return null
#endregion
