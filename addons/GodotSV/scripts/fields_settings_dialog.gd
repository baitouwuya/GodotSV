...
var _default_custom_editor: Control # 自定义默认值编辑器（来自 GDSVEditorRegistry）
...
func _create_default_setting(parent: VBoxContainer) -> LineEdit:
	...
	_populate_type_options(default_array_type_option)
...
func _show_default_editor_for_type(data_type: String) -> void:
	...
	if _default_multi_value_container:
		_default_multi_value_container.visible = false
	# 清理自定义默认值编辑器
	if _default_custom_editor and is_instance_valid(_default_custom_editor):
		_default_custom_editor.queue_free()
		_default_custom_editor = null
	...
	# 检查 GDSVEditorRegistry 自定义编辑器
	var registry: GDSVTypeHandlerRegistry = GDSVTypeHandlerRegistry.get_singleton()
	if registry:
		registry.scan_script_types()
	if GDSVEditorRegistry.has_editor(t):
		_default_text_container.visible = false
		var config: Dictionary = {}
		config["initial_value"] = _default_edit.text.strip_edges() if _default_edit else "0"
		config["row"] = -1
		config["column"] = _current_column
		var editor: Control = GDSVEditorRegistry.create_editor(t, -1, _current_column, config)
		if editor:
			editor.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			_default_editor_root.add_child(editor)
			_default_custom_editor = editor
			if editor.has_signal("value_changed"):
				editor.value_changed.connect(func() -> void:
					if _suppress_default_sync:
						return
					_suppress_default_sync = true
					if editor.has_method("get_value"):
						_default_edit.text = str(editor.get_value())
...
func _sync_default_editor_from_text(data_type_lower: String) -> void:
	...
	# 自定义编辑器同步：值已通过 value_changed 信号同步，无需额外处理
	if _default_custom_editor and is_instance_valid(_default_custom_editor):
		_suppress_default_sync = false
		_update_default_clear_buttons()
		return
...
func _add_option_item_with_meta(option: OptionButton, text: String, meta: String, icon: Texture2D = null) -> int:
...
func _refresh_type_options() -> void:
	if _type_option:
		...
		_field_type_more_index = _add_option_item_with_meta(_type_option, _MORE_ITEM_DEFAULT_TEXT, _OPTION_META_MORE)
		_field_type_custom_index = _field_type_more_index
	if _array_element_type_option:
...
func _populate_type_options(option: OptionButton, exclude_types: PackedStringArray = []) -> void:
	...
	for t in all_types:
		var ts := str(t)
		if ts in exclude_types:
			continue
		sorted_types.append(ts)
	sorted_types.sort()
	...
	for type_name in sorted_types:
		...
		if not handler:
			_add_option_item_with_meta(option, type_name, type_name, icon)
			continue
		...
		if not display_name.is_empty() and display_name != type_name:
			_add_option_item_with_meta(option, "%s (%s)" % [display_name, type_name], type_name, icon)
		else:
			_add_option_item_with_meta(option, type_name, type_name, icon)
