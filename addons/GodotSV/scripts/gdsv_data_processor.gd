...
func load_gdsv_content(content: String, file_path: String = "", options: Dictionary = {}) -> bool:
	...
	# 如果有文件路径，基于扩展名推断分隔符（调用方未显式设置时作为兜底）
...
## TSV 转换为 GDSV（GDSV 默认使用 Tab 分隔，与 TSV 相同）
func _tsv_to_gdsv(tsv_content: String) -> String:
	# GDSV 默认使用 Tab 分隔符，TSV 内容可直接使用
	# 仅清理空行
...
func rename_column(column_index: int, new_name: String) -> bool:
	...
	if column_index < _original_header.size():
		# 替换注解中的字段名部分，保留类型后缀
		var old_annotation: String = _original_header[column_index]
		var colon_pos: int = old_annotation.find(":")
		if old_annotation.begins_with("*"):
			# 必填标记：替换 * 后面到 : 之间的部分
			if colon_pos > 1:
				_original_header[column_index] = "*" + new_name + old_annotation.substr(colon_pos)
			else:
...
func parse_type_annotations(header_row: PackedStringArray) -> Array:
	...
	# 统一使用 GDSVTypeAnnotationParser 解析所有表头格式
	# 支持完整注解语法（name:Type, *name:Type, name:int[0..100], name:enum(a,b) 等）
	# 也兼容简单的 name:type=default 格式（无注解时类型默认为 String）
...
## 根据类型定义更新指定列的表头注解
## 确保保存时 _original_header 包含正确的类型信息
func update_column_annotation(column_index: int, field_name: String, type_definition: Dictionary) -> void:
	if column_index < 0 or column_index >= _original_header.size():
		return

	_original_header[column_index] = _build_annotation_string(field_name, type_definition)

	# 同步 _cleaned_header
	if column_index < _cleaned_header.size():
		_cleaned_header[column_index] = field_name


## 从类型定义字典构建注解字符串（如 "*name:int[0..100]=0"）
func _build_annotation_string(field_name: String, type_def: Dictionary) -> String:
	var annotation: String = ""

	# 必填前缀
	if type_def.get("required", false):
		annotation += "*"

	annotation += field_name

	var data_type: String = str(type_def.get("type", "")).strip_edges()
	if data_type.is_empty() or data_type == "string":
		# 无类型或 string 类型不写注解
		return annotation
	...
	# 枚举值
	if data_type.to_lower() == "enum":
		var enum_values: Variant = type_def.get("enum_values", [])
		if enum_values is Array and not enum_values.is_empty():
			var values_str: String = ",".join(PackedStringArray(enum_values))
			annotation += "(" + values_str + ")"

	# Array 元素类型
	if data_type.to_lower() == "array":
		var element_type: String = str(type_def.get("array_element_type", "")).strip_edges()
		if not element_type.is_empty():
			annotation += "[" + element_type + "]"

	# 数值范围约束
	var has_min: bool = type_def.has("min")
	var has_max: bool = type_def.has("max")
	if has_min or has_max:
		var min_str: String = str(type_def["min"]) if has_min else ""
		var max_str: String = str(type_def["max"]) if has_max else ""
		annotation += "[" + min_str + ".." + max_str + "]"

	# 默认值
	var default_val: String = str(type_def.get("default", "")).strip_edges()
