class_name CSVSchema
extends Resource

## CSV 数据模式定义，用于定义 CSV 文件的整体结构、字段定义和验证规则

## 字段定义字典（字段名 -> CSVFieldDefinition）
...
## 添加字段定义
func add_field(field_name: StringName, field_type: CSVFieldDefinition.FieldType = CSVFieldDefinition.FieldType.TYPE_STRING) -> CSVFieldDefinition:
	var definition := CSVFieldDefinition.new(field_name, field_type)
	field_definitions[field_name] = definition
	return definition


## 获取字段定义
func get_field_definition(field_name: StringName) -> CSVFieldDefinition:
...
func get_required_fields() -> Array:
	...
	for field_name in field_definitions:
		var definition: CSVFieldDefinition = field_definitions[field_name]
